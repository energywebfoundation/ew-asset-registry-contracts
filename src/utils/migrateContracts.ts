...
export async function migrateAssetRegistryContracts(
    ...
    return new Promise<any>(async (resolve, reject) => {
        ...
        const assetConsumingLogicAddress = (await deploy(
            ...
            web3.eth.abi.encodeParameters(['address', 'address'],
                                          [userContractLookup, assetContractLookupAddress]).substr(2),
            { privateKey: privateKeyDeployment },
        )).contractAddress;

        const assetConsumingDBAddress = (await deploy(
            web3,
            (AssetConsumingDBJSON as any).bytecode + web3.eth.abi.encodeParameter('address',
                                                                                  assetConsumingLogicAddress).substr(2),
            { privateKey: privateKeyDeployment },
        )).contractAddress;
        ...
        const assetProducingDBAddress = (await deploy(
            web3,
            (AssetProducingDBJSON as any).bytecode + web3.eth.abi.encodeParameter('address',
                                                                                  assetProducingLogicAddress).substr(2),
            { privateKey: privateKeyDeployment },
        )).contractAddress;
