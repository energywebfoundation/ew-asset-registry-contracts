    ...
    export class AssetContractLookup extends GeneralFunctions
{
    ...
    constructor(web3: Web3, address?: string){
        super(address ? new web3.eth.Contract(AssetContractLookupJSON.abi, address) : new web3.eth.Contract(AssetContractLookupJSON.abi, AssetContractLookupJSON.networks.length > 0 ? (AssetContractLookupJSON.networks[0]) : null))
        this.web3 = web3
    }
	...
	async init(_userRegistry:string,_assetProducingRegistry:string,_assetConsumingRegistry:string,_assetProducingDB:string,_assetConsumingDB:string, txParams?:SpecialTx){

            let transactionParams

            const txData = await this.web3Contract.methods.init(_userRegistry,_assetProducingRegistry,_assetConsumingRegistry,_assetProducingDB,_assetConsumingDB)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.init(_userRegistry,_assetProducingRegistry,_assetConsumingRegistry,_assetProducingDB,_assetConsumingDB)
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.init(_userRegistry,_assetProducingRegistry,_assetConsumingRegistry,_assetProducingDB,_assetConsumingDB)
	...
	async update(_assetProducingRegistry:string,_assetConsumingRegistry:string, txParams?:SpecialTx){

            let transactionParams

            const txData = await this.web3Contract.methods.update(_assetProducingRegistry,_assetConsumingRegistry)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.update(_assetProducingRegistry,_assetConsumingRegistry)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.update(_assetProducingRegistry,_assetConsumingRegistry)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
