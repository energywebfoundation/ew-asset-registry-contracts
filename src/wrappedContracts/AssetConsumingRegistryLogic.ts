    ...
    export class AssetConsumingRegistryLogic extends GeneralFunctions
{
    ...
    constructor(web3: Web3, address?: string){
        super(address ? new web3.eth.Contract(AssetConsumingRegistryLogicJSON.abi, address) : new web3.eth.Contract(AssetConsumingRegistryLogicJSON.abi, AssetConsumingRegistryLogicJSON.networks.length > 0 ? (AssetConsumingRegistryLogicJSON.networks[0]) : null))
        this.web3 = web3
    }
	...
	async createAsset(_smartMeter:string,_owner:string,_active:boolean,_matcher:string[],_propertiesDocumentHash:string,_url:string, txParams?:SpecialTx){

            let transactionParams

            const txData = await this.web3Contract.methods.createAsset(_smartMeter,_owner,_active,_matcher,_propertiesDocumentHash,_url)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.createAsset(_smartMeter,_owner,_active,_matcher,_propertiesDocumentHash,_url)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.createAsset(_smartMeter,_owner,_active,_matcher,_propertiesDocumentHash,_url)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
