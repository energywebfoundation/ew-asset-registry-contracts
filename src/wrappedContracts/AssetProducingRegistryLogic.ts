    ...
    export class AssetProducingRegistryLogic extends GeneralFunctions
{
    ...
    constructor(web3: Web3, address?: string){
        super(address ? new web3.eth.Contract(AssetProducingRegistryLogicJSON.abi, address) : new web3.eth.Contract(AssetProducingRegistryLogicJSON.abi, AssetProducingRegistryLogicJSON.networks.length > 0 ? (AssetProducingRegistryLogicJSON.networks[0]) : null))
        this.web3 = web3
    }
	...
	async addMatcher(_assetId:number,_new:string, txParams?:SpecialTx){
            ...
            const txData = await this.web3Contract.methods.addMatcher(_assetId,_new)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.addMatcher(_assetId,_new)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.addMatcher(_assetId,_new)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
	async createAsset(_smartMeter:string,_owner:string,_active:boolean,_matcher:string[],_propertiesDocumentHash:string,_url:string,_numOwnerChanges:number, txParams?:SpecialTx){

            let transactionParams

            const txData = await this.web3Contract.methods.createAsset(_smartMeter,_owner,_active,_matcher,_propertiesDocumentHash,_url,_numOwnerChanges)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.createAsset(_smartMeter,_owner,_active,_matcher,_propertiesDocumentHash,_url,_numOwnerChanges)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.createAsset(_smartMeter,_owner,_active,_matcher,_propertiesDocumentHash,_url,_numOwnerChanges)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
	async setMarketLookupContract(_assetId:number,_marketContractLookup:string, txParams?:SpecialTx){

            let transactionParams

            const txData = await this.web3Contract.methods.setMarketLookupContract(_assetId,_marketContractLookup)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.setMarketLookupContract(_assetId,_marketContractLookup)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.setMarketLookupContract(_assetId,_marketContractLookup)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
	...
	async setBundleActive(_assetId:number,_active:boolean, txParams?:SpecialTx){
            ...
            const txData = await this.web3Contract.methods.setBundleActive(_assetId,_active)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.setBundleActive(_assetId,_active)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.setBundleActive(_assetId,_active)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
	async saveSmartMeterRead(_assetId:number,_newMeterRead:number,_lastSmartMeterReadFileHash:string, txParams?:SpecialTx){

            let transactionParams

            const txData = await this.web3Contract.methods.saveSmartMeterRead(_assetId,_newMeterRead,_lastSmartMeterReadFileHash)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.saveSmartMeterRead(_assetId,_newMeterRead,_lastSmartMeterReadFileHash)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.saveSmartMeterRead(_assetId,_newMeterRead,_lastSmartMeterReadFileHash)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
	async removeMatcher(_assetId:number,_remove:string, txParams?:SpecialTx){
            ...
            const txData = await this.web3Contract.methods.removeMatcher(_assetId,_remove)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.removeMatcher(_assetId,_remove)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.removeMatcher(_assetId,_remove)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
	...
	async init(_dbAddress:string,param1:string, txParams?:SpecialTx){
            ...
            const txData = await this.web3Contract.methods.init(_dbAddress,param1)
            .encodeABI()
            ...
            if(txParams){
                ...
                if(!txParams.gas){
                    ...
                    try{
                    gas = await this.web3Contract.methods.init(_dbAddress,param1)
                        .estimateGas({ from: txParams ? txParams.from : (await this.web3.eth.getAccounts())[0]} )
                    } catch(ex){
            ...
            if (transactionParams.privateKey !== '') {
                ...
                return await this.web3Contract.methods.init(_dbAddress,param1)
                    .send({from: transactionParams.from, gas: transactionParams.gas}) 
            }
	}
