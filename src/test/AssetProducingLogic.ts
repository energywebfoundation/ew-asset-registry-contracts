...
describe("AssetProducingLogic", () => {
  ...
  const assetOwnerPK =
    "0xfaab95e72c3ac39f7c060125d9eca3558758bb248d1a4cdc9c1b7fd3f91a4485";
  const assetOwnerAddress = web3.eth.accounts.privateKeyToAccount(assetOwnerPK)
    .address;

  const assetSmartmeterPK =
    "0x2dc5120c26df339dbd9861a0f39a79d87e0638d30fdedc938861beac77bbd3f5";
  const assetSmartmeter = web3.eth.accounts.privateKeyToAccount(
    assetSmartmeterPK
  ).address;

  const matcherPK =
    "0xc118b0425221384fe0cbbd093b2a81b1b65d0330810e0792c7059e518cea5383";
  const matcher = web3.eth.accounts.privateKeyToAccount(matcherPK).address;

  const assetSmartmeter2PK =
    "0x554f3c1470e9f66ed2cf1dc260d2f4de77a816af2883679b1dc68c551e8fa5ed";
  const assetSmartMeter2 = web3.eth.accounts.privateKeyToAccount(
    assetSmartmeter2PK
  ).address;

  it("should deploy the contracts", async () => {
    const userContracts = await migrateUserRegistryContracts(
      web3,
      privateKeyDeployment
    );
    ...
    Object.keys(deployedContracts).forEach(async key => {
      ...
      if (key.includes("AssetContractLookup")) {
        assetContractLookup = new AssetContractLookup(
          web3 as any,
      ...
      if (key.includes("AssetConsumingDB")) {
        assetConsumingDB = new AssetConsumingDB(
          web3 as any,
          deployedContracts[key]
        );
      ...
      if (key.includes("AssetConsumingRegistryLogic")) {
        ...
        tempBytecode =
          "0x" + (AssetConsumingRegistryLogicJSON as any).deployedBytecode;
      }

      if (key.includes("AssetProducingDB")) {
        assetProducingDB = new AssetProducingDB(
          web3 as any,
          deployedContracts[key]
        );
      ...
      if (key.includes("AssetProducingRegistryLogic")) {
        ...
        tempBytecode =
          "0x" + (AssetProducingRegistryLogicJSON as any).deployedBytecode;
      }
  ...
  it("should have the right owner", async () => {
    assert.equal(
      await assetProducingLogic.owner(),
      assetContractLookup.web3Contract._address
    );
  });
  ...
  it("should have the right userContractLookup", async () => {
    assert.equal(
      await assetProducingLogic.db(),
      assetProducingDB.web3Contract._address
    );
  });
  ...
  it("should return empty asset when smart meter is not onboarded yet", async () => {
    const deployedAsset = await assetProducingLogic.getAssetBySmartMeter(
      assetSmartmeter
    );
  ...
  it("should be able to deactive an asset", async () => {
    ...
    const eventInactive = (await assetProducingLogic.getAllLogAssetSetInactiveEvents(
      { fromBlock: tx.blockNumber - 1, toBlock: tx.blockNumber + 1 }
    ))[0];
  ...
  it("should fail when trying to call update", async () => {
    ...
    try {
      await assetProducingLogic.update(
        "0x7110d0f07be70fc2a6c84fe66bf128593b2102fb",
        { privateKey: privateKeyDeployment }
      );
    } catch (ex) {
      failed = true;
