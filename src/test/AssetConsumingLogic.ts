...
describe("AssetConsumingLogic", () => {
  ...
  it("should have the right owner", async () => {
    assert.equal(
      await assetConsumingLogic.owner(),
  ...
  it("should have the right db", async () => {
    assert.equal(
      await assetConsumingLogic.db(),
      assetConsumingDB.web3Contract._address
    );
  });
  ...
  it("should return empty asset", async () => {
    const emptyAsset = await assetConsumingLogic.getAssetBySmartMeter(
      assetSmartmeter
    );
  ...
  it("should return asset by smartmeter", async () => {
    const deployedAsset = await assetConsumingLogic.getAssetBySmartMeter(
      assetSmartmeter
    );
